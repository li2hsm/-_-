...
/**
 * 测试多个activity本地绑定服务
 * 测试结果，一个本地服务可以供多个activity组件绑定。
 */
public class TestMuiltLocalBindServiceActivity extends AppCompatActivity {
    ...
    public void bindLocalService() {
        Intent intent = new Intent(TestMuiltLocalBindServiceActivity.this, NormalServices.class);
        mServiceConnection = new ServiceConnection() {

            @Override
            public void onServiceConnected(ComponentName name, IBinder service) {
                //绑定成功之后，用语传递参数
                Log.w("绑定服务", "第二个activity绑定成功。");
                NormalServices.CustomIBinder customIBinder = (NormalServices.CustomIBinder) service;
                customIBinder.getWords("第二个activity绑定成功");
            }
